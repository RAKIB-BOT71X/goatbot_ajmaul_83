// set bash title
process.stdout.write("\x1b]2;GoatBot V2 - Made by NTKhang,\x1b\x5c");
...
const { log: _log, logColor: _logColor, getPrefix: _getPrefix, createOraDots: _createOraDots, jsonStringifyColor: _jsonStringifyColor, getText: _getText, convertTime: _convertTime, colors: _colors, randomString: _randomString } = global.utils || {};
const log = _log || { info: console.log, warn: console.warn, err: console.error, error: console.error, master: console.log };
const logColor = _logColor || ((color, msg) => console.log(msg));
const getPrefix = _getPrefix || (() => global.GoatBot?.config?.prefix || ".");
const createOraDots = _createOraDots || ((msg) => ({ _start: () => {}, _stop: () => {}, text: msg }));
const jsonStringifyColor = _jsonStringifyColor || JSON.stringify;
const getText = _getText || ((key1, key2, ...args) => args.join(" ") || key2 || key1);
const convertTime = _convertTime || ((ms) => ms + "ms");
const colors = _colors || { green: (t) => t, yellowBright: (t) => t, blueBright: (t) => t, hex: () => (t) => t };
const randomString = _randomString || (() => Math.random().toString(36).slice(2));
...
let subTitle = `GoatBot@${currentVersion} - Made by mdajmaul (goatbot_ajmaul_83)`;
const subTitleArray = [];
...
const author = ("Made by mdajmaul (goatbot_ajmaul_83)");
const srcUrl = ("Source code: https://github.com/mdajmaul/goatbot_ajmaul_83");
...
async function startBot(loginWithEmail) {
        console.log(gradient("#f5ab00", "#ff4b2b")(createLine("START LOGGING IN", true)));
        ...
        (function loginBot(appState) {
                ...
                login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
                        if (typeof global.utils.applyGlobalFontStyle === 'function') global.utils.applyGlobalFontStyle(api);
                        ...
                        log.info("BOT NICK NAME", global.GoatBot.config.nickNameBot || "GoatBot Ajmaul_83");
                        log.info("MAINTAINER", "NTKhang (Original) | mdajmaul (Fork)");
